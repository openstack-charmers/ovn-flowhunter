#!/usr/bin/env python3
#
# Detect and delete duplicated flows in routers defined in OVN.
# Steps:
#
# - Get the output of 'ovn-sbctl lflow-list <datapath>', where <datapath> is a
#   router's name (e.g. zuul-tests_router).
# - Group the list of logical flows dropping the "action=..." tail.
# - For each flow of the previous list that has 2 or more ocurrences the
#   associated port is deleted. To identify the right port the following steps
#   are taken:
#   + Get the output of 'ovn-nbctl list logical-switch-port' where each section
#     (separated by an empty line) is a port.
#   + Get the MAC and IP addresess from the flow that wants to be deleted.
